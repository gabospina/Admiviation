...
function logIn(form) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.trim() === "success") { // Corrected response check
                // window.location.href = "home.php";
                window.location.href = "/admviation/home.php";
            } else {
                $("#loginError").text("Invalid username or password."); // Generic message
            }
        },
        error: function (xhr) {
...
$(document).ready(function () {
    ...
    //             dataType: "json",
    //             success: function (response) {
    //                 if (response.status === "not taken") {
    //                     $(that).removeClass("input-error").addClass("input-success");
    //                     $("#usernameTaken").text("");
    //                 } else {
    //                     $(that).removeClass("input-success").addClass("input-error");
    //                     $("#usernameTaken").html(" <strong>" + response.message + "</strong>");
    //                 }
    //             },
    //             error: function (xhr, status, error) {
    //                 console.error("AJAX Error (check_username):", status, error, xhr.responseText);
    //                 $(that).removeClass("input-success").addClass("input-error");
    //                 $("#usernameTaken").html(" <strong>Error checking username</strong>");
    //             }
    //         });
    //     }
    // });
