...
function setContractEvents(init) {
	...
	//  ======= BELOW CAUSE ERROR - Error parsing contracts data: TypeError: $(...).sortable is not a function at setContractEvents (contractfunctions.js:88:18) ???? ============
	...
	//  ======= ABOVE CAUSE ERROR - Error parsing contracts data:
	...
	$.ajax({
		...
		success: function (result) {
			if (result.success) {
				...
				tableContractList += "<thead><tr><th>Contract Name</th><th>Customer ID</th><th>Color</th><th>Actions</th></tr></thead><tbody>";

				for (var i = 0; i < contracts.length; i++) {
					tableContractList += "<tr><td>" + contracts[i].contract + "</td><td>" + contracts[i].customerid + "</td><td>" + contracts[i].color + "</td><td>" +
	...
	$.ajax({
		...
		success: function (result) {
			if (result.success) {
				...
				for (var i = 0; i < contracts.length; i++) {
					tableContractList += "<tr><td>" + contracts[i].contract + "</td><td>" + contracts[i].customer_name + "</td><td>" + contracts[i].color + "</td><td>" +
						"<button class='btn btn-danger btn-sm deleteContractBtn' data-id='" + contracts[i].contractid + "'>-</button></td></tr>";
				}
...
$(document).on('click', '#submitNewCustomerBtn', function () {
	//alert("Button works!");
...
$(document).on('click', '#submitNewContractBtn', function () {
	...
	var customerName = $('#newContractCustomerSelect option:selected').text(); // Get customer name
	...
	$.ajax({
		...
		data: {
			...
			pilot_ids: pilotIds,
			color: color,
			customer_name: customerName // Pass customer name
		},
		success: function (response) {
			if (response.success) {
				alert("Contract added successfully!");
				// Optionally, refresh the contracts list or redirect
			} else {
...
// loadCrafts() function: Loads the list of crafts from get_all_crafts.php and populates the #newContractCraftSelect select box.

// loadPilots() function: Loads the list of pilots from get_all_pilots.php and populates the #newContractPilotSelect select box.