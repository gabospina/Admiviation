...
// 1. Retrieve Account Information (Company Name)
// if (isset($_SESSION["account"]) && is_numeric($_SESSION["account"])) {
//     $account_id = (int)$_SESSION["account"];

//     $query = $mysqli->prepare("SELECT name FROM account WHERE id = ?"); // Select only the account name
...
//         if ($accountData) {
//             $accountName = $accountData["name"]; // Store the account name
//         } else {
//             error_log("Account information not found for account ID: " . $account_id);
//         }

//         $query->close();
//     } else {
//         error_log("Database query error (account): " . $mysqli->error);
//     }
// } else {
//     error_log("Invalid or missing account ID in session.");
// }

// 3. Check $_SESSION['company_id'] is an integer
if (isset($_SESSION["company_id"]) && is_numeric($_SESSION["company_id"])) {
    ...
    if ($query->execute()) {
        ...
        error_log("Database query error (company): " . $mysqli->error);
    }
} else {
    error_log("Invalid or missing company ID in session.");
}

// 2. Retrieve User Information (Username)
...
if (isset($_SESSION["HeliUser"]) && is_numeric($_SESSION["HeliUser"])) {
    ...
    if ($query->execute()) {
        ...
        error_log("Database query error (user): " . $mysqli->error);
    }
} else {
...
<head>
	...
	<?php
  		// $query = $mysqli->query("SELECT a.name, ai.nationality FROM account a INNER JOIN account_info ai ON ai.id=a.id WHERE a.id=$_SESSION[account]");
  		// $accountRes = $query->fetch_assoc();
  		// $accountNationality = $accountRes["nationality"];
  		// $logbookName = $mysqli->query("SELECT logbook FROM account_info WHERE id=$_SESSION[account]")->fetch_assoc()["logbook"];

		//  PROBLEM WITH VARIABLE $accountNationality and $logbookName - line 58 -->
		echo '<script>var accountNationality = "'.htmlspecialchars($accountNationality, ENT_QUOTES, 'UTF-8').'";
  							logbookName = "'.htmlspecialchars($logbookName, ENT_QUOTES, 'UTF-8').'";
  				</script>';
		...
		switch($page){
			...
			case "pilots":
				echo '
					...
					<script src="assets/js/pilotfunctions.js"></script>
				';
			break;
			case "contracts":
				echo '
					<script src="assets/js/universalfunctions.js"></script>
					<script src="assets/js/loginfunctions.js"></script>
					<script src="assets/lib/spectrum.js"></script>
					<link type="text/css" rel="stylesheet" href="assets/css/spectrum.css">
					<script src="assets/js/contractfunctions.js"></script>
				';
			break;
			case "hangar":
				echo '
					...
					<script src="assets/lib/dropzone.js"></script>
					<script src="assets/lib/moment.min.js"></script>
					<script src="assets/js/hangarfunctions.js"></script>
				';
			break;
			case "crafts":
				echo'<script src="assets/js/universalfunctions.js"></script>
					<script src="assets/js/loginfunctions.js"></script>
					<script src="assets/js/craftfunctions.js"></script>';
			break;
			case "statistics":
				// var maxDay = '.$mysqli->query("SELECT max_in_day FROM account_info WHERE id=$_SESSION[account]")->fetch_assoc()["max_in_day"].';
				// var maxSeven = '.$mysqli->query("SELECT max_last_7 FROM account_info WHERE id=$_SESSION[account]")->fetch_assoc()["max_last_7"].';
				// var max28 = '.$mysqli->query("SELECT max_last_28 FROM account_info WHERE id=$_SESSION[account]")->fetch_assoc()["max_last_28"].';
				//  Addressing undefined variables
				...
				echo'<script src="assets/lib/jquery.flot.min.js"></script>
					...
					<script src="assets/js/statsfunctions.js"></script>';
			break;
			case "training":
				echo'<script src="assets/lib/moment.min.js"></script>
					...
					<script src="assets/js/universalfunctions.js"></script>
					<script src="assets/js/loginfunctions.js"></script>
					<script src="assets/js/trainingfunctions.js"></script>';
			break;
			case "scala":
				echo'<script src="assets/lib/moment.min.js"></script>
...
<body>
  ...
  <div class="sidebar">
    <ul class="sidebar-list">
      ...
      <a href="hangar.php">
        <li class="sidebar-item"><i class="fa fa-lg fa-cogs"></i> My Hangar/hangar</li>
      </a>
		...
		// 	        <li class="sidebar-item"><i class="fa fa-lg fa-wrench"></i> Account Settings<br/> <small style="padding-left: 30px;">Names, Time Limitations</small></li>
		// 	      </a>';
      	// }
      ?>
    </ul>
