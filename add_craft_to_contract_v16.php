...
$craftId = isset($_POST["craft_id"]) ? (int)$_POST["craft_id"] : 0;

// Validate input data
if ($contractId <= 0 || $craftId <= 0) {
...
$checkSql = "SELECT c.id AS contract_id, craft.id AS craft_id
             ...
             JOIN crafts craft ON craft.company_id = cust.company_id
             WHERE c.id = ? AND craft.id = ? AND cust.company_id = ?";
...
$checkStmt->bind_param("iii", $contractId, $craftId, $companyId);
$checkStmt->execute();
...
$checkSql = "SELECT id FROM contract_crafts WHERE contract_id = ? AND craft_type_id = ?";
$checkStmt = $mysqli->prepare($checkSql);
...
$checkStmt->bind_param("ii", $contractId, $craftId);
$checkStmt->execute();
$checkStmt->store_result();
...
$insertSql = "INSERT INTO contract_crafts (contract_id, craft_type_id) VALUES (?, ?)";
...
$insertStmt->bind_param("ii", $contractId, $craftId);
if ($insertStmt->execute()) {
    ...
    $updateCraftStmt->bind_param("ii", $contractId, $craftId);
